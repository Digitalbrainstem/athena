...
|------|-------------------|
| Little Learner | "Tap the red bridge" → taps → confetti + companion celebrates |
| Explorer | "How much fencing for 12×8 field?" → answers 40m → fence appears around field |
| Adventurer | "Balance the equation: _H₂ + O₂ → _H₂O" → enters 2,1,2 → reaction animates |
| Scholar | "Calculate orbital insertion Δv" → enters 3.2 km/s → ship adjusts trajectory smoothly |
| Master | "Predict protein fold conformation" → submits model → molecular dynamics sim runs |
...
| **Player-created** | Master-tier players can design quests (Atlas-validated before serving) |
...
**Explorer:** Recipe following with measurement
...
**Scholar:** Complex synthesis and analysis
...
|------|-------------|----------------------|
| Little Learner | Blocks stack, triangles are stable | Balance, gravity, basic shapes |
| Explorer | Supports needed for bridges, roofs need slopes | Load distribution, geometry |
| Adventurer | Stress, strain, load calculations | F=ma, material properties, structural integrity |
| Scholar | Beam deflection, moment of inertia, resonance | Advanced mechanics, calculus applications |
| Master | Finite element analysis concepts, novel materials | Advanced engineering, research methodology |

### Building Example: The Bridge Quest (Explorer Tier)
...
| **Debt/credit** | Borrow from the bank for large purchases (Adventurer+) | Interest, compound growth, risk |

### Economy by Tier

**Little Learner:** Coin counting — 1, 2, 5, 10 coins. "How many to buy the apple?"

**Explorer:** Arithmetic with money. Percentage discounts. Unit pricing. Budgeting.

**Adventurer:** Supply/demand simulation. Market trends. Investment vs. spending. Opportunity cost.

**Scholar:** Macroeconomics. Monetary policy. Game theory (prisoner's dilemma in multiplayer trading). Econometric modeling.

**Master:** Complex systems modeling. Policy design. Market manipulation detection. International trade theory.
...
|------|---------------|----------------|
| Explorer (6-10) | Visual/block coding | Direct robot: forward, turn, pick up, drop |
| Explorer (8-10) | Simple text commands | Loops: "Repeat 10: plant, step forward" |
| Adventurer | Full text, Python-like syntax | Variables, functions, conditionals, arrays |
| Scholar | Real Python/JS in sandbox | Data structures, algorithms, file I/O, networking |
| Master | Systems programming | Distributed systems, ML models, compilers, OS concepts |

### Code Forge Examples

**Explorer (block coding):**
```
[MOVE FORWARD] → [TURN LEFT] → [MOVE FORWARD] → [PICK UP]
```
Robot navigates maze and collects ore.

**Adventurer (text coding):**
```python
...
### "Build a Spaceship and Reach the Moon" (Scholar)
...
### "Cure the Village Plague" (Adventurer)
...
### "Run the Kingdom for a Year" (Adventurer–Scholar)
